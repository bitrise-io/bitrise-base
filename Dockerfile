...
RUN wget -q https://storage.googleapis.com/golang/go1.7.5.linux-amd64.tar.gz -O go-bins.tar.gz
...
#
# Install Bitrise CLI
RUN curl -fL https://github.com/bitrise-io/bitrise/releases/download/1.5.2/bitrise-$(uname -s)-$(uname -m) > /usr/local/bin/bitrise
...
ENV BITRISE_DOCKER_REV_NUMBER_BASE v2017_01_11_1
CMD bitrise --version
